...
BGtest <- function(data, y_data, X_data, order = 1) {
  ...
  X0 <- as.matrix(c)
  BP_df <- cbind(data[y_data], X0)
  BP_mat <- to_matrix(BP_df, colnames(BP_df))
  
  
  # Regress
  R2_stat <- ols(BP_mat, y_data, X_data = names(X0))$R2
...
t <- BGtest(data = gdp_data, y_data = "delta_p", X_data = c("Year", "Realgdp", "Realcons", "Realinvs", "Realgovt", "Realdpi", "CPI_U", "M1", "Tbilrate", "Unemp", "Pop", "Infl", "Realint"))
