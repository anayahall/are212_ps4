...
## I dropped 
covariates <- c("yr_qtr", "Realgdp", "Realcons", "Realinvs", "Realgovt", "Realdpi", "CPI_U", "M1", "Tbilrate", "Unemp", "Pop", "Infl", "Realint")

model_1 <- ols(gdp_data, 
               y_data = "delta_p", 
               X_data =c("yr_qtr", "Realgdp", "Realcons", "Realinvs", "Realgovt", "Realdpi", "CPI_U", "M1", "Tbilrate", "Unemp", "Pop", "Infl", "Realint"))
...
BGtest(data = gdp_data, y_data = "delta_p", X_data = c("yr_qtr", "Realgdp", "Realcons", "Realinvs", "Realgovt", "Realdpi", "CPI_U", "M1", "Tbilrate", "Unemp", "Pop", "Infl", "Realint")) %>% knitr::kable()
...
BPtest(data = gdp_data, y_data = "delta_p", X_data = c("yr_qtr", "Realgdp", "Realcons", "Realinvs", "Realgovt", "Realdpi", "CPI_U", "M1", "Tbilrate", "Unemp", "Pop", "Infl", "Realint")) %>% knitr::kable()
...
DWtest(data = gdp_data, y_data = "delta_p", X_data = c("yr_qtr", "Realgdp", "Realcons", "Realinvs", "Realgovt", "Realdpi", "CPI_U", "M1", "Tbilrate", "Unemp", "Pop", "Infl", "Realint")) %>% knitr::kable()
...
newres <- prais$vars$e
```
Most of the signs on the parameter estimates are the same, but the magnitude has changed. Some significance results have changed as well!
...
``` {r test_loop for C matrix, include = FALSE}
rho <- .3
t <- 15
G <- matrix(NA, nrow=t, ncol =t)
  for (i in 1:t) {
...
# Adjust fgls_sc function above to include an option to omit first observation???
# ---> I DON'T THINK I DID THIS RIGHT!! PLEASE CHECK :)
...
I don't think I dropped the first observation correctly, but at present it doesn't seem to have changed the results dramatically.....

``` {r plot CO results}

#plot(gdp_data$Year, newres2)

ggplot(gdp_data, aes(x=Year, y=newres2)) + geom_line()
