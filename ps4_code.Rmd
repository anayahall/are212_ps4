...
``` {r include = FALSE}
test <- subset(gdp_data, Year <= 1960)
test <- select(test, Year, Unemp)


lag_var <- "Unemp"
test$shifted <- NA
  for (i in 1:nrow(test)) {
    if (i == 1) test$shifted[[i]] = 0
    else 
      test$shifted[[i]] = lag_var[i-1]}
...
# shift <- function(data, lag_var, shift_by = -1) {
#   
#   data$shifted <- NA
# 
#   for (i in 1:nrow(data)) {
#     if (i == 1) data$shifted[i] = 0
#     else data$shifted[i] = data$lag_var[i-1]}
# 
#   
# }
...
BGtest(data = gdp_data, y_data = "delta_p", X_data = c("Year", "Realgdp", "Realcons", "Realinvs", "Realgovt", "Realdpi", "CPI_U", "M1", "Tbilrate", "Unemp", "Pop", "Infl", "Realint"))
